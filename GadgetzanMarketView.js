...
function marketCollectionCardClicked(cardDisplay) {
	...
	if (marketDeckSize<30 && cardDisplay.card.amount > 0 && funds>=price) {
		...
		if (!cardFoundInDeck) {
			marketDeck.push({name:cardDisplay.card.name,id:cardDisplay.card.id,rarity:cardDisplay.card.rarity,manaCost:cardDisplay.card.manaCost,theClass:cardDisplay.card.theClass,amount:1,cardId:cardDisplay.card.cardId});
			cardDisplay.card.amount--;
			removeFromCollection(cardDisplay.card.id,1,marketCollection);
			loadCardAmountText(cardDisplay, 50,-45,-255, 0);
			if (cardDisplay.card.amount<=0)
...
function marketCollectionBarClicked(cardDisplay) {
	...
	addToCollection({name:cardDisplay.card.name,id:cardDisplay.card.id,rarity:cardDisplay.card.rarity,manaCost:cardDisplay.card.manaCost,theClass:cardDisplay.card.theClass,amount:1,cardId:cardDisplay.card.cardId},marketCollection,null,1);
	for (var i=0;i<8;i++) {
